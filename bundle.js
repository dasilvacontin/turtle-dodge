...
var MAP_EDGE = 10;
var CELL_EDGE = Math.min(Math.floor(window.innerWidth / MAP_EDGE), Math.floor(window.innerHeight / MAP_EDGE));
canvas.width = canvas.height = MAP_EDGE * CELL_EDGE;

var map = new Array(MAP_EDGE + 1);
for (var i = 0; i < MAP_EDGE + 1; ++i) {
  var row = new Array(MAP_EDGE);
  row.fill(0);
  map[i] = row;
}
...
var players = [];
function init() {
  for (var k = 1; k <= PLAYER_COUNT; ++k) {
    var _randomEmptyInRange = randomEmptyInRange(1, 1, MAP_EDGE - 2, MAP_EDGE - 2),
        _randomEmptyInRange2 = (0, _slicedToArray3.default)(_randomEmptyInRange, 2),
        _i2 = _randomEmptyInRange2[0],
        j = _randomEmptyInRange2[1];

    map[_i2][j] = k;
    players.push({ i: _i2, j: j });
  }
}
init();
...
function isCellEmpty(i, j) {
  return i >= 0 && j >= 0 && i < MAP_EDGE && j < MAP_EDGE && map[i][j] === 0;
}
...
function logic() {
  ...
  players.forEach(function (player, k) {
    ...
    var playerControls = controls[k];
    dirs.some(function (dir) {
      var _dirInc$dir = (0, _slicedToArray3.default)(dirInc[dir], 2),
          _i = _dirInc$dir[0],
          _j = _dirInc$dir[1];

      if (kbd.isKeyDown(playerControls[dir]) && isCellEmpty(i + _i, j + _j)) {
        map[i][j] = 0;
        map[i + _i][j + _j] = k + 1;
        player.i += _i;
        player.j += _j;
  ...
  // move rocks
  for (var _i3 = MAP_EDGE - 1; _i3 >= 0; _i3--) {
    for (var j = MAP_EDGE - 1; j >= 0; j--) {
      var cell = map[_i3][j];
      if (cell >= 0) continue;

      // rock
      map[_i3][j] = 0;
      if (cell === -1) map[_i3 + 1][j] = cell; // down
      else if (cell === -2) map[_i3][j + 1] = cell; // right
    }
  }

  // spawn rocks
  var SPAWN_CHANCE = 0.04;
  for (var _i4 = 0; _i4 < MAP_EDGE; ++_i4) {
    map[_i4][0] = Math.random() < SPAWN_CHANCE ? -2 : 0;
  }
  for (var _j2 = 0; _j2 < MAP_EDGE; ++_j2) {
    map[0][_j2] = Math.random() < SPAWN_CHANCE ? -1 : 0;
  }
}
...
function render() {
  for (var _i5 = 0; _i5 < MAP_EDGE; ++_i5) {
    var _row = map[_i5];
    for (var j = 0; j < MAP_EDGE; ++j) {
      var cell = _row[j];
      var color = fillForCell[cell];

      // TODO: circle?
      ctx.globalAlpha = cell !== 0 ? 1 : 0.9;
      ctx.fillStyle = color;
      ctx.fillRect(j * CELL_EDGE, _i5 * CELL_EDGE, CELL_EDGE, CELL_EDGE);
    }
  }
}

var MSPF = 200;
var lastTurn = new Date() - MSPF;
function gameloop() {
...
},{"@dasilvacontin/keyboard":1,"babel-runtime/helpers/slicedToArray":4}]},{},[59]);
